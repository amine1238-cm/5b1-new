...
from resource_platform.database_backend import (SQLiteBackend, PostgreSQLBackend, BackendNotImplementedError, DatabaseErrorCategory, parse_database_target)
...
class Increment5ATests(unittest.TestCase):
    ...
    def test_postgresql_selection_fails_before_connection(self):
        ...
        selected=backend(settings); self.assertIsInstance(selected,PostgreSQLBackend)
        with self.assertRaises(BackendNotImplementedError): selected.connect()
