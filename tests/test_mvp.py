...
class MVPTests(unittest.TestCase):
    ...
    def booking_resource(self):
        return self.db().execute("SELECT * FROM resources WHERE status='available' LIMIT 1").fetchone()
    def test_successful_booking(self):
        r=self.booking_resource(); _,cookie=self.login('employee@example.test','EmployeePassphrase-2026!'); body=b'start_at=2026-09-10T09%3A00&end_at=2026-09-10T10%3A00&purpose=Site+visit'; status,_=self.request(f'/resources/{r["id"]}/book','POST',body,cookie); self.assertEqual(status[0],'303 See Other'); db=self.db(); self.assertEqual(db.execute("SELECT COUNT(*) FROM bookings").fetchone()[0],1); db.close()
    def test_invalid_time_range(self):
        r=self.booking_resource(); _,c=self.login('employee@example.test','EmployeePassphrase-2026!'); st,d=self.request(f'/resources/{r["id"]}/book','POST',b'start_at=2026-09-10T10%3A00&end_at=2026-09-10T09%3A00&purpose=x',c); self.assertEqual(st[0],'409 Conflict'); self.assertIn(b'Start time',d)
    ...
    def test_overlapping_booking(self):
        r=self.booking_resource(); _,c=self.login('employee@example.test','EmployeePassphrase-2026!'); body=b'start_at=2026-09-10T09%3A00&end_at=2026-09-10T10%3A00&purpose=x'; self.assertEqual(self.request(f'/resources/{r["id"]}/book','POST',body,c)[0][0],'303 See Other'); st,d=self.request(f'/resources/{r["id"]}/book','POST',b'start_at=2026-09-10T09%3A30&end_at=2026-09-10T10%3A30&purpose=x',c); self.assertEqual(st[0],'409 Conflict'); self.assertIn(b'already booked',d)
    def test_booking_after_cancelled(self):
        r=self.booking_resource(); _,c=self.login('employee@example.test','EmployeePassphrase-2026!'); body=b'start_at=2026-09-10T09%3A00&end_at=2026-09-10T10%3A00&purpose=x'; st,_=self.request(f'/resources/{r["id"]}/book','POST',body,c); bid=int(dict((k,v) for k,v in st[1]).get('Location','').split('/')[-1]); self.assertEqual(self.request(f'/bookings/{bid}/cancel','POST',b'',c)[0][0],'303 See Other'); self.assertEqual(self.request(f'/resources/{r["id"]}/book','POST',body,c)[0][0],'303 See Other')
    ...
    def test_employee_only_own_bookings(self):
        r=self.booking_resource(); _,c=self.login('employee@example.test','EmployeePassphrase-2026!'); self.request(f'/resources/{r["id"]}/book','POST',b'start_at=2026-09-10T09%3A00&end_at=2026-09-10T10%3A00&purpose=x',c); st,d=self.request('/bookings',cookie=c); self.assertEqual(st[0],'200 OK'); self.assertIn(b'Bookings',d)
    def test_admin_booking_access_and_audit(self):
        r=self.booking_resource(); _,c=self.login(); self.assertEqual(self.request(f'/resources/{r["id"]}/book','POST',b'start_at=2026-09-10T09%3A00&end_at=2026-09-10T10%3A00&purpose=x',c)[0][0],'303 See Other'); db=self.db(); self.assertEqual(db.execute("SELECT COUNT(*) FROM audit_logs WHERE action='booking.create'").fetchone()[0],1); db.close()
    ...
    def test_two_simultaneous_booking_attempts(self):
        ...
        body=b'start_at=2026-09-10T14%3A00&end_at=2026-09-10T15%3A00&purpose=Concurrent+test'
